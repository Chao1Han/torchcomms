...
class XCCLException : public std::exception {
 public:
  XCCLException(XcclApi& api, const std::string& message, ze_result_t result);

  const char* what() const noexcept override;
  ze_result_t getResult() const;
...
class TorchCommXCCL : public TorchCommBackend,
 ...
 protected:
  ...
  void returnEvent(ze_event_handle_t event);
  void abortXcclComm();
  ...
  xcclDataType_t getXcclDataType(const at::Tensor& tensor);
 ...
 private:
  ...
  struct RedOpRAII {
    /* implicit */ RedOpRAII(xcclRedOp_t op);

    // Constructor for Premulsum Reduction
    explicit RedOpRAII(
        const ReduceOp& op,
        const xcclComm_t comm,
        const xcclDataType_t dataType,
    ...
    operator xcclRedOp_t() const {
      return xcclRedOp_;
    }

    xcclRedOp_t xcclRedOp_{xcclMaxRedOp};
    xcclComm_t comm_{nullptr};
  ...
  explicit TorchCommXCCL(const xcclComm_t xccl_comm);

  // Private utility methods
  size_t wordSize(xcclDataType_t type) const;
  ...
  // Member variables
  xcclComm_t xccl_comm_{};
  at::Device device_;
  int comm_size_{};
  int rank_{};
  CommOptions options_;
  size_t max_event_pool_size_{};
  xpuStream_t internal_stream_{};
  ze_event_handle_t
  ...
  std::queue<ze_event_handle_t> event_pool_;
  ...
  static void XPU_CB graphCleanupCallback(void* userData);
