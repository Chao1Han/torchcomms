...
class TorchCommXCCLBootstrap {
 ...
 private:
  xcclUniqueId exchangeUniqueId(std::string_view name);
  xcclUniqueId exchangeUniqueIdStore();
  xcclUniqueId exchangeUniqueIdTCPStore(std::string_view name);
  bool isTCPStoreEnabled();
  void cleanupTCPStore(xcclComm_t xccl_comm);
