...
void TorchCommXCCL::finalize() {
  ...
  // Clean up event pool
  {
    ...
    while (!event_pool_.empty()) {
      ze_event_handle_t event = event_pool_.front();
      event_pool_.pop();
      XPU_CHECK(
  ...
  if (dependency_event_) {
    XPU_CHECK(
        xpu_api_,
        xpu_api_->eventDestroy(dependency_event_),
  ...
  if (internal_stream_) {
    XPU_CHECK(
        xpu_api_,
        xpu_api_->streamDestroy(internal_stream_),
...
ze_event_handle_t TorchCommXCCL::getEvent() {
  std::lock_guard<std::mutex> lock(event_pool_mutex_);
  ...
  ze_result_t result = xpu_api_->eventCreate(&event);
  if (result != ZE_RESULT_SUCCESS) {
    TC_LOG(ERROR) << "Failed to create event: " << xpu_api_->getErrorString(result);
    throw std::runtime_error("Failed to create XPU event");
  }
  
  return event;
}

void TorchCommXCCL::returnEvent(ze_event_handle_t event) {
  std::lock_guard<std::mutex> lock(event_pool_mutex_);
  ...
  if (event_pool_.size() < max_event_pool_size_) {
    ...
    // Destroy the event if pool is full
    XPU_CHECK(
        xpu_api_, 
        xpu_api_->eventDestroy(event), 
        "Failed to destroy excess event");
  }
}
