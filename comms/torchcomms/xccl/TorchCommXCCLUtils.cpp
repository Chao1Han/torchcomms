  ...
  switch (dataType) {
    case onecclFloat16:
      createPreMulSum<at::Half, onecclFloat16>(
          &onecclRedOp_, factor, comm, xccl_api_.get());
      break;
    case onecclFloat32:
...
TorchCommXCCL::RedOpRAII::~RedOpRAII() {
  if (comm_) {
    xccl_api_->redOpDestroy(onecclRedOp_, comm_);
  }
}
...
xpuStream_t TorchCommXCCL::getOperationStream(bool async_op) {
  if (async_op) {
    ...
    XPU_CHECK(
        xpu_api_,
        xpu_api_->eventRecord(dependency_event_, current_stream),
        "Failed to record dependency event");

    XPU_CHECK(
        xpu_api_,
        xpu_api_->streamWaitEvent(internal_stream_, dependency_event_, 0),
