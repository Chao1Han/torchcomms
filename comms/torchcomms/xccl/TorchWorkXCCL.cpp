...
TorchWorkXCCL::WorkStatus TorchWorkXCCL::checkStatus() {
  ...
  if (!start_completed_time_.has_value()) {
    ze_result_t start_status = comm_->getXpuApi()->eventQuery(start_event_);

    if (start_status == ZE_RESULT_SUCCESS) {
      ...
      state_ = WorkStatus::INPROGRESS;
        ...
        start_status != ZE_RESULT_NOT_READY &&
        start_status != ZE_RESULT_ERROR_UNSUPPORTED_FEATURE) {
  ...
  ze_result_t end_status = comm_->getXpuApi()->eventQuery(end_event_);

  if (end_status == ZE_RESULT_SUCCESS) {
    ...
    inputTensors_.clear();
  } else if (end_status == ZE_RESULT_NOT_READY) {
  ...
  } else if (end_status != ZE_RESULT_ERROR_UNSUPPORTED_FEATURE) {
